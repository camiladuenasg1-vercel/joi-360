...
export function Badge({ label, color = "primary", icon }) {
  const COLORS = {
    primary:  "bg-[#e2dfff] text-[#3525cd] border-[#c3c0ff]/50",
...
export function SectionTitle({ children, action, onAction }) {
  return (
    <div className="flex justify-between items-center mb-3">
      <p className="text-[11px] font-black text-[#777587] uppercase tracking-widest">{children}</p>
      {action && <button onClick={onAction} className="text-[11px] font-bold text-[#3525cd]">{action}</button>}
    </div>
  );
}
...
export function BigNumber({ prefix, value, suffix, sub, color = "text-[#1b1b24]" }) {
  return (
    <div>
      {sub && <p className="text-[11px] font-bold text-[#777587] uppercase tracking-widest mb-1">{sub}</p>}
      <div className="flex items-baseline gap-1">
        {prefix && <span className="text-xl font-bold text-[#777587]">{prefix}</span>}
        <span className={`text-5xl font-black tracking-tight leading-none ${color}`}>{value}</span>
        {suffix && <span className="text-lg text-[#777587] font-semibold">{suffix}</span>}
      </div>
    </div>
  );
}
...
/** Stat chip grid item */
export function StatChip({ label, value, icon, color = "bg-[#f0ecf9]" }) {
  return (
    <div className={`${color} rounded-2xl p-3`}>
      {icon && <span className="material-symbols-outlined fill text-[#3525cd] text-base block mb-1">{icon}</span>}
      <p className="text-[10px] font-bold text-[#777587] uppercase">{label}</p>
      <p className="font-black text-[#1b1b24] text-sm mt-0.5">{value}</p>
    </div>
  );
}
...
export function ConfigHint({ icon, label }) {
  return (
    <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl bg-[#f0ecf9] border border-[#e2dfff]">
      <span className="material-symbols-outlined fill text-[#3525cd] text-sm">{icon}</span>
  ...
  return (
    <div className="px-5 pb-8 text-center pt-4">
      <div className="w-16 h-16 rounded-3xl bg-[#f0ecf9] flex items-center justify-center mx-auto mb-4">
        <span className="material-symbols-outlined text-[#c7c4d8] text-3xl">block</span>
      </div>
      <p className="text-[#1b1b24] font-black text-lg">Módulo no disponible</p>
      <p className="text-[#777587] text-sm mt-2 max-w-[260px] mx-auto leading-relaxed">
        <b>{name}</b> no está habilitado en <b>{mundo?.nombre}</b> por tu comunidad.
      </p>
      <p className="text-[#c7c4d8] text-xs mt-4">Contacta a tu administrador para activarlo.</p>
    </div>
  );
}
