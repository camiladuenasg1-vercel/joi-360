      ...
      <div className="relative w-full sm:max-w-[420px] bg-white rounded-t-[32px] sm:rounded-[32px] max-h-[85vh] overflow-y-auto">
        ...
        {mode === "success" ? (
          <div className="px-6 pt-4 pb-2 text-center">
            ...
            <h2 className="text-2xl font-black text-[#1b1b24]">¡Bienvenido a {m.nombre}!</h2>
            <p className="text-[#777587] text-sm mt-2 leading-relaxed">Tu billetera en esta comunidad ya está activa.</p>
          </div>
        ) : (
          <div className="px-6 pt-2 pb-2">
            <div className="flex items-center gap-3">
              <div className="w-14 h-14 rounded-2xl flex items-center justify-center flex-shrink-0 overflow-hidden text-white font-black text-xl" style={{ background: m.color || "#3525cd" }}>
                {m.logoUrl ? <img src={m.logoUrl} alt="" className="w-full h-full object-cover" /> : m.nombre[0]}
              </div>
              <div className="min-w-0">
                <h2 className="text-xl font-black text-[#1b1b24] leading-tight">{m.nombre}</h2>
                <span className="text-[10px] font-black text-[#3525cd] uppercase tracking-wider">{m.vertical}</span>
              </div>
            </div>
            <p className="text-[#464555] text-sm mt-4 leading-relaxed">
        ...
        {chips.length > 0 && (
          <div className="px-6 py-4">
            <p className="text-[10px] font-bold text-[#777587] uppercase tracking-widest mb-3">
            ...
            <div className="flex flex-wrap gap-2">
              {chips.map(c => (
                <div key={c.id} className={`flex items-center gap-1.5 ${c.bg} rounded-full pl-2 pr-3 py-1.5`}>
                  ...
                  <span className="text-xs font-semibold text-[#1b1b24]">{c.label}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="px-6 pb-4 flex flex-wrap gap-4">
          <span className="text-[#777587] text-xs flex items-center gap-1">
            ...
            <b className="text-[#1b1b24]">{comerciosCount}</b> comercios
          </span>
          <span className="text-[#777587] text-xs flex items-center gap-1">
        ...
        <div className="px-6 pb-8 pt-2">
          <button
            ...
            style={{ background: `linear-gradient(135deg,${m.color || "#3525cd"},#4f46e5)`, boxShadow: `0 4px 14px ${m.color || "#3525cd"}40` }}>
            {loading ? (
          ...
          {mode === "preview" && (
            <button onClick={onClose} className="w-full text-center text-[#777587] text-sm font-semibold mt-3">Cancelar</button>
          )}
        </div>
      </div>
    </div>
