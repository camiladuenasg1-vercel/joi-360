...
- **`home-render-config.md`** (14-sep-2026) — el detalle específico de Home (`Hub.jsx`) hacia adelante: el grafo de navegación completo, el gate exacto de cada bloque de Home, `/pay`/`/activity`/`/profile` (las 3 pantallas de la barra inferior que no son un `moduleId`), y el checklist de cotejo contra el panel RedPontis (Admin) del propio proyecto de Salvador.
