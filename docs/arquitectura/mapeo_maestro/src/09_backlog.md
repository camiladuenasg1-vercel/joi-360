...
- [x] **Subsidio** — v1.0.0, código completo. Diseño: solo RedPontis acredita (recomendación aplicada, sin objeción de Camila), uno a la vez, sin carga masiva todavía. Ledger propio `subsidios`, sin tocar `wallets.balance` ni `mover_saldo_wallet` — el consumo real (gastarlo en una compra) queda para una v1.1. Panel real en Usuarios → detalle de la persona (`SubsidioPanel`, visible solo si el mundo tiene la capacidad activa), con monto/categorías/vigencia y rastro de auditoría (`acreditado_por`). **Bloqueado hasta correr `supabase-subsidio.sql`.**
- [ ] Asistencia y Facturación/Crédito quedan fuera de este checklist (ver clasificación 🟡/🔴 arriba) — necesitan una definición de producto o un convenio externo antes de construir, no una decisión que se pueda tomar de paso.

**Checklist cerrado (26-ago) — las 6 capacidades 🟢 construibles del plan original están en v1.0.0.**

**✅ Actualización 28-ago:** las 4 migraciones (`supabase-turnos-food-court.sql`, `supabase-reservas.sql`, `supabase-estacionamiento.sql`, `supabase-subsidio.sql`) **se corrieron y se verificaron en vivo de punta a punta** — se activaron las 6 capacidades en Jockey Plaza CR-01 (`mundo-3ox15t`) y se recorrió cada template en la superapp contra su tabla real (incluyó crear y borrar una reserva de verdad). `FLAG_DEV_MAP` de turnos/reservas/estacionamiento/subsidio pasó de `in_progress` a `ready` (commit `4c9851b`). Bug de timezone encontrado y corregido en `ReservasTemplate` en la misma pasada. Detalle en `docs/arquitectura/migraciones_aplicadas.md` y en el corte v1.3 del Historial de Tareas (`#246`/`#248`).
...
**~~Pendiente operativo — 4 archivos SQL escritos, todavía sin correr.~~ ✅ Corridos y verificados el 28-ago** (`supabase-turnos-food-court.sql`, `supabase-reservas.sql`, `supabase-estacionamiento.sql`, `supabase-subsidio.sql`). Las 4 tablas existen en prod con las columnas exactas de sus `.sql`. Nota para futuras migraciones: ninguna de las 4 terminó con `notify pgrst, 'reload schema';` (regla del anexo §10) — el reload se hizo por el script combinado que sí lo incluía. Ver `migraciones_aplicadas.md`.
