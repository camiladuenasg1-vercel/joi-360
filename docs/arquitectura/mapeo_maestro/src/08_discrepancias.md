...
`organizadores.password` se insertaba sin hash, y las credenciales del Panel de Merchant ni siquiera llegaban a Supabase. **Track B — hecho en código, pendiente de correr `supabase-auth-organizador-merchant.sql`:** triggers `hash_organizador_password` / `hash_merchant_panel_password` (bcrypt, mismo patrón que `admin_users`), RPCs `verificar_login_organizador` / `verificar_login_merchant` (security definer), `REVOKE SELECT` de `password`/`*_hash`/`pos_pin*`/`sponsor_password` para el rol `anon`. El cliente (`organizadorLogin`/`merchantLogin`) ya usa las RPCs con un fallback local transitorio que hay que quitar una vez migrado. Falta la pasada de la **app nativa + `joi-pos-backend`** (B4: el nativo manda el PIN como campo `operador`).
