...
| **E · Sync de config** — 🟡 parcial (07-sep) | ✅ `syncCatalogRemote` propaga `version` (real semver) + `status` (`futuro` para `MODULOS_PROXIMAMENTE`, antes "1.0"/"activo" para las 22) · ✅ `defaultModuleState` exportado y usado en "Agregar del catálogo global" (antes `serviciosActivos:{}` → 0 flags sincronizados) · ✅ copy de `CatalogoProductos` honesta (era "fuente maestra"). **Falta:** limpieza de los 28 flags fantasma en `capacity_feature_flags` (SQL guardado, pendiente de correr) · persistir `dev_status`/`api` a Supabase · `ensureGlobalFlag` con DELETE. | `supabase.js`, `store.js`, `MundoDetail.jsx`, `CatalogoProductos.jsx` | Fase 0 | Medio | 🟡 en curso |
