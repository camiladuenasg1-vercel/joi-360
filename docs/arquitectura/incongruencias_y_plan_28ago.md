...
| **G · Paridad Operador + BNPL RPC** — 🟡 parcial (07-sep) | ✅ **BNPL cronograma+interés**: `BNPL_INTERES_POR_CUOTAS` + `cronogramaBNPL()` exportados de `store.js` (única fuente); `OperadorApp.jsx` y `Fronts.jsx` los usan; la copia del Operador ahora honra `frecuencia:"personalizada"` + `dias_personalizados` y usa fecha local (antes ignoraba lo primero y usaba UTC → cuotas distintas app vs POS). La superapp (`Module.jsx:cronogramaDe`, otro repo) ya lo hacía bien — debe espejar `store.js`. ✅ **`mover_cashback_wallet` + `verificar_login_sponsor`** versionadas: `.sql` reconstruidos de prod en `docs/arquitectura/`. **Falta:** botón de cierre de turno en `CobrarPanel` (5 `pos_turnos` abiertos desde 04-ago) · web accesos → aviso al apoderado (`user_notifications`) + validación de estado de pulsera (`nfc_bands`) para paridad con el nativo · vincular pulsera con reglas iguales. | `OperadorApp.jsx`, `Fronts.jsx`, `joi-pos-backend/routes/*`, `store.js`, nuevos `.sql` | parcial de B | Alto | 🟡 en curso |
