...
| **D · Docs de frente** — ✅ hecho (07-sep, nota de autoridad + correcciones puntuales) | `01/03/04/05/06/07` llevan ahora una nota "⚠️ escrito el 12-ago, describe mecánica no estado; autoridad = `02_registro_capacidades.md`" + los errores factuales corregidos (`MODULOS_PROXIMAMENTE` 11→3, `06` sincronizado con `08` sobre fix-234, tabla de tokens de `07` marcada pre-rebrand con las divergencias reales, Cashback/Suscripciones/precompra ya existen, 2 modos de Operador). **Nota:** es la vía "nota + fix puntual", no una regeneración completa desde código — eso queda para cuando se automatice el build de estos docs. | `docs/arquitectura/mapeo_maestro/src/*` | ninguna | Medio | ✅ |
| **E · Sync de config** — 🟢 casi cerrado (09-sep) | ✅ `syncCatalogRemote` propaga `version` real + `status` — **aplicado en prod** (`capacities` ya no es "1.0" plano) · ✅ `defaultModuleState` sembrando `serviciosActivos` al agregar del catálogo global · ✅ copy de `CatalogoProductos` honesta · ✅ **28 flags fantasma borrados** (96→68) · ✅ **`world_feature_flags` de `mundo-3ox15t` sembrados** (0→16, Track I). **Falta solo:** persistir `dev_status`/`api` de feature flags a Supabase (hoy solo `localStorage`) + `ensureGlobalFlag` con DELETE (sync bidireccional) — cambio más grande, sin urgencia. | `supabase.js`, `store.js`, `MundoDetail.jsx`, `CatalogoProductos.jsx` | Fase 0 | Medio | 🟢 |
...
| **G · Paridad Operador + BNPL RPC** — 🟢 cerrado (09-sep) | ✅ **BNPL cronograma+interés**: `BNPL_INTERES_POR_CUOTAS` + `cronogramaBNPL()` exportados de `store.js` (única fuente); `OperadorApp.jsx` y `Fronts.jsx` los usan; la copia del Operador ahora honra `frecuencia:"personalizada"` + `dias_personalizados` y usa fecha local (antes ignoraba lo primero y usaba UTC → cuotas distintas app vs POS). La superapp (`Module.jsx:cronogramaDe`, otro repo) ya lo hacía bien — debe espejar `store.js`. ✅ **`mover_cashback_wallet` + `verificar_login_sponsor`** versionadas: `.sql` reconstruidos de prod en `docs/arquitectura/`. ✅ **Cierre de turno en `CobrarPanel`**: `cerrarTurnoPOSRemote(turnoId, montoDeclarado)` (misma lógica que `turno-cerrar.js` — lo esperado sale de `transactions.turno_id`, no del cliente); acordeón "Cerrar turno / cuadre de caja" con arqueo (esperado/declarado/diferencia) y "Abrir turno nuevo"; cobrar/recargar/QR se deshabilitan sin turno abierto. ✅ **Web accesos → paridad nativa**: `registrarAccesoRemote` avisa al apoderado (`user_notifications`) si el sujeto es dependiente; `resolverUsuarioParaAcceso` valida estado de pulsera (`nfc_bands`: OTRO_MUNDO/BLOQUEADA/NO_VINCULADA/VENCIDA) antes de resolver por código de cuenta, igual que `bandResolver.js`. ✅ **Vincular pulsera**: ya estaba en paridad — `bands-link.js` fue alineado con `vincularNfcBandRemote` en el fix de QA previo (chequeo de estado + bandita activa del usuario + vigencia). Accesos `1.0.1 → 1.0.2`. | `OperadorApp.jsx`, `Fronts.jsx`, `supabase.js`, `store.js` | parcial de B | Alto | 🟢 |
...
| **I · QA E2E** — 🟡 code-review + estado en prod (07-sep · gap de precompra cerrado 09-sep) | ✅ **Precompra** (`crearPedidoEvento`): el dinero va por `comprarProductosLive` → `mover_saldo_wallet` (RPC con candado + validación de stock/saldo/restricciones); solo inserta `event_product_orders` si `r.ok`. ✅ **Error path cerrado (09-sep)**: si el insert del pendiente de retiro falla tras cobrar, se reintenta 1 vez y, si aun así falla, `crearPedidoEvento` devuelve `{ ok:false, motivo:"orden_no_registrada", cobrado:true }` — la UI ya NO manda a reintentar (cobraría doble), muestra "tu pago se procesó, acércate al stand" y limpia el carrito. ✅ **Devolución** (`procesar_devolucion`): RPC versionada (`add-procesar-devolucion.sql`), step-up con contraseña de admin. ✅ Las 6 capacidades: render + una escritura real (reserva) ya verificados el 28-ago. ⚠️ **Hallazgo:** `world_feature_flags` para las 6 caps de `mundo-3ox15t` = **0 filas** (se agregaron antes del fix de Track E) → `supabase-seed-flags-3ox15t.sql` (ya corrido). **Falta:** ejecución E2E en vivo del cobro de cada cap + precompra + devolución, con data de prueba (evento con productos de precompra, ticket de devolución) — mejor en sesión interactiva. | superapp + admin (browser) | A, C | Medio | 🟡 code-verified |
