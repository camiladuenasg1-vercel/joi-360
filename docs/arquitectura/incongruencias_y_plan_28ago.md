...
| **I · QA E2E** — 🟡 code-review + estado en prod (07-sep) | ✅ **Precompra** (`crearPedidoEvento`): el dinero va por `comprarProductosLive` → `mover_saldo_wallet` (RPC con candado + validación de stock/saldo/restricciones); solo inserta `event_product_orders` si `r.ok`. Único gap: si el insert falla tras cobrar, el usuario queda cobrado sin registro de retiro — falta error path. ✅ **Devolución** (`procesar_devolucion`): RPC versionada (`add-procesar-devolucion.sql`), step-up con contraseña de admin. ✅ Las 6 capacidades: render + una escritura real (reserva) ya verificados el 28-ago. ⚠️ **Hallazgo:** `world_feature_flags` para las 6 caps de `mundo-3ox15t` = **0 filas** (se agregaron antes del fix de Track E) → `supabase-seed-flags-3ox15t.sql` para arreglarlo. **Falta:** ejecución E2E en vivo del cobro de cada cap + precompra + devolución, con data de prueba (evento con productos de precompra, ticket de devolución) — mejor en sesión interactiva. | superapp + admin (browser) | A, C | Medio | 🟡 code-verified |
