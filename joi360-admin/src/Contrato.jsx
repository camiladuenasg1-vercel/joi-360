...
export function ContratoView() {
  ...
  return (
    <div style={{ fontFamily: "Georgia, serif", maxWidth: 800, margin: "0 auto", padding: "40px 60px", lineHeight: 1.7, color: "#0b1c30" }}>
      ...
      <div className="no-print" style={{ position: "fixed", top: 16, right: 16, display: "flex", gap: 8 }}>
        <button onClick={() => window.print()} style={{ padding: "8px 16px", background: "#0035b9", color: "white", border: "none", borderRadius: 6, cursor: "pointer", fontFamily: "Inter, sans-serif" }}>
          Imprimir / Guardar PDF
        </button>
        <button onClick={() => window.close()} style={{ padding: "8px 16px", background: "#f0f0f0", border: "none", borderRadius: 6, cursor: "pointer", fontFamily: "Inter, sans-serif" }}>
          Cerrar
        </button>
      </div>

      <div style={{ textAlign: "center", marginBottom: 40, borderBottom: "3px solid #0035b9", paddingBottom: 24 }}>
        <div style={{ fontSize: 11, letterSpacing: 2, color: "#747686", marginBottom: 8, fontFamily: "monospace" }}>REDPONTIS S.A.C. · CONTRATO DE SERVICIO JOI 360</div>
        <h1 style={{ fontSize: 22, fontWeight: "bold", margin: "8px 0", fontFamily: "Inter, sans-serif" }}>Propuesta de Acuerdo Comercial</h1>
        <div style={{ fontSize: 11, color: "#747686", fontFamily: "monospace" }}>VERSIÓN TENTATIVA · DOCUMENTO GENERADO EL {today.toUpperCase()}</div>
      </div>
        ...
        <tbody>
          {[
            ...
            <tr key={k}>
              ...
              <td style={{ padding: "6px 0", fontSize: 13, borderBottom: "1px solid #e5eeff" }}>{v || "—"}</td>
            </tr>
          ))}
        </tbody>
      </table>

      <h2 style={{ fontSize: 15, borderLeft: "4px solid #0035b9", paddingLeft: 12, marginBottom: 16, fontFamily: "Inter, sans-serif" }}>Tipo de acuerdo comercial</h2>
      <div style={{ background: "#eff4ff", padding: "16px 20px", borderRadius: 8, marginBottom: 24 }}>
        <div style={{ fontSize: 14, fontWeight: "bold", textTransform: "uppercase", color: "#0035b9", letterSpacing: 1, fontFamily: "monospace", marginBottom: 8 }}>{ac.tipo || "Por definir"}</div>
      ...
      <h2 style={{ fontSize: 15, borderLeft: "4px solid #0035b9", paddingLeft: 12, marginBottom: 16, fontFamily: "Inter, sans-serif" }}>Módulos contratados ({mods.length})</h2>
      <table style={{ width: "100%", borderCollapse: "collapse", marginBottom: 24, fontSize: 12 }}>
        ...
        <tbody>
          {mods.map(mm => {
            ...
            return (
              <tr key={mm.id} style={{ borderBottom: "1px solid #e5eeff" }}>
                <td style={{ padding: "7px 10px", fontFamily: "Inter, sans-serif" }}>{cat?.name || mm.id}</td>
                <td style={{ padding: "7px 10px", color: "#747686", fontFamily: "monospace", fontSize: 11 }}>{cat?.category}</td>
                {gratuito ? (
                  <td colSpan={3} style={{ padding: "7px 10px", textAlign: "right", color: "#0e7c43", fontWeight: "bold", fontSize: 11 }}>GRATUITO POR EL MOMENTO</td>
                ) : (<>
          ...
          <tr style={{ background: "#eff4ff", fontWeight: "bold" }}>
      ...
      <h2 style={{ fontSize: 15, borderLeft: "4px solid #0035b9", paddingLeft: 12, marginBottom: 16, fontFamily: "Inter, sans-serif" }}>Términos y condiciones</h2>
      <div style={{ fontSize: 12, lineHeight: 1.8, color: "#444655" }}>
        {[
      ...
      <div style={{ marginTop: 60, display: "flex", justifyContent: "space-between", gap: 60 }}>
        <div style={{ flex: 1, textAlign: "center", borderTop: "1px solid #0b1c30", paddingTop: 12 }}>
          <div style={{ fontSize: 12, fontFamily: "monospace", letterSpacing: 1 }}>REDPONTIS S.A.C.</div>
          <div style={{ fontSize: 11, color: "#747686", marginTop: 4 }}>RUC 20999888777</div>
        </div>
        <div style={{ flex: 1, textAlign: "center", borderTop: "1px solid #0b1c30", paddingTop: 12 }}>
          <div style={{ fontSize: 12, fontFamily: "monospace", letterSpacing: 1 }}>{m.entidadLegal?.toUpperCase() || "SPONSOR"}</div>
          <div style={{ fontSize: 11, color: "#747686", marginTop: 4 }}>RUC {m.ruc || "—"} · {m.apoderadoNombre || m.representanteLegal || "Representante Legal"}</div>
        </div>
      </div>

      <div style={{ textAlign: "center", marginTop: 40, fontSize: 10, color: "#c4c5d7", fontFamily: "monospace" }}>
